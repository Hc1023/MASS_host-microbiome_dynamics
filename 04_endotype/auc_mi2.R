...
### microbe
vars_m = paste0("D1_mi_",c("HHV.4","HCMV"))

auc_mat1 = pred_fun(var = "APACHEII_24h")
auc_mat2 = pred_fun(var = vars_CTSg, m = 'GLMNET')
auc_mat3 = pred_fun(var = vars_m, m = 'GLM')
auc_mat4 = pred_fun(var = c(vars_CTSg, vars_m), m = 'GLMNET')
...
### microbe
vars_m = paste0("D4_mi_",c("HHV.4","HCMV"))

auc_mat1_D4 = pred_fun(var = "APACHEII_24h", tps = 28)
auc_mat2_D4 = pred_fun(var = vars_CTSg, tps = 28, m = 'GLMNET')
auc_mat3_D4 = pred_fun(var = vars_m, tps = 28, m = 'GLM')
auc_mat4_D4 = pred_fun(var = c(vars_CTSg, vars_m), tps = 28, m = 'GLMNET')
...
### microbe
vars_m = paste0("D7_mi_",c("HHV.4","HCMV"))

auc_mat1_D7 = pred_fun(var = "APACHEII_24h", tps = 28)
auc_mat2_D7 = pred_fun(var = vars_CTSg, tps = 28, m = 'GLMNET')
auc_mat3_D7 = pred_fun(var = vars_m, tps = 28, m = 'GLM')
auc_mat4_D7 = pred_fun(var = c(vars_CTSg, vars_m), tps = 28, m = 'GLMNET')
...
pdf(paste0("Outputs/04_auc_all_mi2.pdf"), width = 7, height = 2.5)
print(p)
dev.off()
...
if(F){
  ...
  res1$microbe[grepl('mi', res1$Inputs)] = c('HHV.4,HCMV')
  res1$model = 'GLM'
  res1$model[grepl('CTS', res1$Inputs)] = 'GLMNET'
  
  write.csv(res1, 'Outputs/Supplementary_data_auc_main.csv',
            row.names = F)
...
for (i in c(3,7,28)) {
  ...
  if(fmode == 0){
    var = "APACHEII_24h"
    var = stable_vars_CTSg
    var = stable_vars_m
    var = c(stable_vars_CTSg, stable_vars_m)
  ...
  for(i_iter in 1:n_iter){
    ...
    if(m == "GLMNET"){
      ...
      pred_prob <- as.numeric(
        predict(fit, newx = X_test, s = "lambda.1se", type = "response")
      )
    }
  ...
  auc_list[[paste0("Mortality",i,"d")]] = auc_values
  print(mean(auc_values))
  # cat("\n", outcome, "\n")
  # print(table(df_G1[[outcome]]))
  # print(tapply(df_G1[[var]], df_G1[[outcome]], summary))
...
auc_df <- do.call(
  rbind,
  lapply(names(auc_list), function(nm) {
    v <- auc_list[[nm]]
    data.frame(
      outcome = nm,
      auc_mean = mean(v, na.rm = TRUE),
      auc_lwr  = quantile(v, 0.025, na.rm = TRUE),
      auc_upr  = quantile(v, 0.975, na.rm = TRUE)
    )
  })
)
