...
auc_mat3 = pred_fun(var = vars_m, m = 'GLMNET')
...
auc_mat3_D4 = pred_fun(var = vars_m, tps = 28, m = 'GLMNET')
...
auc_mat3_D7 = pred_fun(var = vars_m, tps = 28, m = 'GLMNET')
...
pdf(paste0("Outputs/04_auc_all_miall.pdf"), width = 7, height = 2.5)
print(p)
dev.off()
...
if(F){
  ...
  res1$model = 'GLMNET'
  res1$model[grepl('APA', res1$Inputs)] = 'GLM'
  
  write.csv(res1, 'Outputs/Supplementary_data_auc_miall.csv',
            row.names = F)
